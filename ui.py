        ...
        with gr.Column(elem_id=f"{id_part}_prompt_container", scale=6):
            with gr.Row():
                with gr.Column(scale=80):
                    with gr.Row():
                        prompt = gr.Textbox(label="Prompt", elem_id=f"{id_part}_prompt", show_label=False, lines=3, placeholder="Prompt (press Ctrl+Enter or Alt+Enter to generate)",value='masterpiece,best quality,1girl,(highly detailed skin:1.2),(full body shot topless facing camera),NSFW,(nude),delicate,finely detailed,intricate details,(photorealistic:1.4),tits,pink_tits,shinning,4k,high-res,Korean K-pop idol,vulva,detailed skin texture,(ulzzang-6500-v1.1:0.6),ultra detailed,<lora:breastinclassBetter:0.4>')
...
def create_sampler_and_steps_selection(choices, tabname):
    if opts.samplers_in_dropdown:
        with FormRow(elem_id=f"sampler_selection_{tabname}"):
            ...
            steps = gr.Slider(minimum=1, maximum=150, step=1, elem_id=f"{tabname}_steps", label="Sampling steps", value=11)
    else:
