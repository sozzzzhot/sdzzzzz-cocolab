        ...
        with gr.Column(elem_id=f"{id_part}_prompt_container", scale=6):
            with gr.Row():
                with gr.Column(scale=80):
                    with gr.Row():
                        prompt = gr.Textbox(label="Prompt", elem_id=f"{id_part}_prompt", show_label=False, lines=3, placeholder="Prompt (press Ctrl+Enter or Alt+Enter to generate)", elem_classes=["prompt"],value='(full body shot topless facing camera),NSFW,(nude),masterpiece,best quality,delicate,finely detailed,intricate details,(photorealistic:1.4),best quality , (photorealistic:1.4),tits,pink_tits,shinning,4k, high-res,best quality,Korean K-pop idol,vulva,detailed skin texture,(ulzzang-6500-v1.1:0.6),ultra detailed, lustrous skin,flat chest, (small breast:1),skinny body, white skin, ((erotic, sexy, horny)) ultra high resolution, highly detailed CG unified 8K wallpapers,physics-based rendering,cinematic lighting,((good anatomy:1.2)),detailed areolas, detailed nipples, detailed breasts,(extremely detailed pussy),(smooth arms:1.2), (clean arms:1.2), (smallbreast:1),<lora:perkyBreasts_v10:0.2>, (full body portrait), (solo focus:1.2),nudity, blush, highres,professional lighting, photon mapping, radiosity,pale skin,white skin,clean skin,nikon RAW photo,8k,Fujifilm XT3,small_breast,<lora:breastinclassBetter:0.4>')

            with gr.Row():
                with gr.Column(scale=80):
                    with gr.Row():
                        negative_prompt = gr.Textbox(label="Negative prompt", elem_id=f"{id_part}_neg_prompt", show_label=False, lines=3, placeholder="Negative prompt (press Ctrl+Enter or Alt+Enter to generate)", elem_classes=["prompt"],value='easynegative,verybadimagenegative_v1.3,paintings, sketches, (worst quality:2), (low quality:2), (normal quality:2), normal quality, ((monochrome)), ((grayscale)), skin spots, acnes, skin blemishes, age spot, glans, extra fingers, fewer fingers, Big breasts,huge breasts.bad_hand,wierd_hand,malformed_hand,malformed_finger,paintings, sketches,skin spots, skin blemishes, age spot, glans,extra fingers,fewer fingers,long neck, oil paintings,((eye shadow)),bad_posture,wierd_posture,poorly Rendered face,poorly drawn face,poor facial details,poorly drawn ,hands,poorly,rendered hands,low resolution,bad composition,mutated body parts,blurry image,disfigured,oversaturated,bad anatomy,deformed body features')
...
def create_sampler_and_steps_selection(choices, tabname):
    if opts.samplers_in_dropdown:
        with FormRow(elem_id=f"sampler_selection_{tabname}"):
            sampler_index = gr.Dropdown(label='Sampling method', elem_id=f"{tabname}_sampling", choices=[x.name for x in choices], value=choices[-3].name, type="index")
            steps = gr.Slider(minimum=1, maximum=150, step=1, elem_id=f"{tabname}_steps", label="Sampling steps", value=10)
    else:
        with FormGroup(elem_id=f"sampler_selection_{tabname}"):
            steps = gr.Slider(minimum=1, maximum=150, step=1, elem_id=f"{tabname}_steps", label="Sampling steps", value=10)
            sampler_index = gr.Radio(label='Sampling method', elem_id=f"{tabname}_sampling", choices=[x.name for x in choices], value=choices[-3].name, type="index")
